...
export default function SavingsPlanDetailsPage({ 
  ...
  const formatTimeRemaining = (timeToMaturity: number) => {
    ...
    const hours = Math.floor((timeToMaturity % (24 * 60 * 60)) / (60 * 60));
    const minutes = Math.floor((timeToMaturity % (60 * 60)) / 60);
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      case 'created': return <Coins className="w-4 h-4 text-blue-600" />;
      ...
      <div className="pt-[10vh] text-black pb-32">
        ...
        >
          ...
          <div className="flex items-center justify-between py-2 px-1 mb-6">
            ...
            <h1 className="text-xl font-bold flex-1 text-center mr-12">Plan Details</h1>
          </div>
          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-2xl font-bold text-gray-800">{savingDetails.name}</h2>
                <Badge 
                  variant={savingDetails.isActive ? "default" : "secondary"}
                  className={`${savingDetails.isActive 
                    ? 'bg-green-100/80 text-green-800 border-green-200' 
                    : 'bg-gray-100/80 text-gray-800 border-gray-200'
                  } backdrop-blur-sm`}
                >
                  {savingDetails.isActive ? 'Active' : 'Completed'}
                </Badge>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-white/20 backdrop-blur-sm rounded-lg p-3 border border-white/30">
                  ...
                  <p className="text-lg font-bold text-gray-800">
                    {savingDetails.amountFormatted} {savingDetails.tokenId}
                  </p>
                </div>
                ...
                <div className="bg-white/20 backdrop-blur-sm rounded-lg p-3 border border-white/30">
                  ...
                  <p className="text-lg font-bold text-gray-800">
                    {savingDetails.interestFormatted} {savingDetails.tokenId}
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium text-gray-700">Maturity Progress</span>
            </div>
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm font-semibold text-gray-800">
                  {formatTimeRemaining(savingDetails.timeToMaturity)}
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-600">Matures On</p>
                <p className="text-sm font-semibold text-gray-800">
                  {new Date(savingDetails.maturityTime * 1000).toLocaleDateString()}
                </p>
              </div>
            </div>
          </Card>
            ...
            <Card className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-3 shadow-lg">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 bg-orange-100/80 backdrop-blur-sm rounded-lg flex items-center justify-center border border-orange-200/50">
                  <span className="text-xs font-medium text-orange-700">CELO</span>
                </div>
                <span className="text-sm text-gray-600">Chain</span>
              </div>
              <p className="text-sm font-semibold text-gray-800">Celo Network</p>
            </Card>
